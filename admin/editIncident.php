...
<head>
  ...
  <a href="addIncident.php"><strong>Back to Incident Manager</strong></a>
  <br><br>
  ...
  <div class="center-wrapper">
    ...
    <form method="post">
      ...
      <input type="text" name="customerID" value="<?= $incident ? htmlspecialchars($incident['customerID']) : '' ?>"
        required>

      <label>Product Code:</label>
      <input type="text" name="productCode" value="<?= $incident ? htmlspecialchars($incident['productCode']) : '' ?>"
        required>
      ...
      <input type="date" name="dateOpened"
        value="<?= $incident ? htmlspecialchars(date('Y-m-d', strtotime($incident['dateOpened']))) : '' ?>" required>

      <label>Date Closed:</label>
      <input type="date" name="dateClosed"
        value="<?= $incident ? htmlspecialchars(date('Y-m-d', strtotime($incident['dateClosed']))) : '' ?>" required>
      ...
      <textarea name="description" rows="5"
        required><?= $incident ? htmlspecialchars($incident['description']) : '' ?></textarea> <br>
  ...
  <!-- --------- INCIDENTS LIST DISPLAY --------- -->
    <?php if (!empty($incidents)): ?>
      ...
      <table border="1" cellpadding="10" style="text-align: center;">
        ...
        <!-- Loop through all incidents and display in table -->
        <?php foreach ($incidents as $i): ?>
          <tr>
            ...
            <td>
              ...
              <a href="deleteIncident.php?id=<?= $i['incidentID'] ?>"
